...
<div class="chart">
    <select class="nombreElement" id="tipoProducto" name="tipoProducto_id" form="añadirProducto">
        ...
        @forelse(TipoProducto::select('*')->get() as $producto)
            <option
                @if(($productoEditar!=null ? $productoEditar->producto->tipoProducto_id : -1) == $producto->id)
                     selected
                @endif
                value="{{$producto->id}}">{{ ucfirst($producto->nombre) }}</option>
        @empty

        @endforelse
    </select>
    <input value="{{$productoEditar!=null ? $productoEditar->producto->titulo : ''}}" class="nombreElement" type="text" id="nombre" name="titulo"
...
<button class="editButton">{{$btnText}}</button>
</form>
</div>
{{--    GESTOR DE PRODUCTOS    --}}
...
<div class="contenedor">
    <div class="contenedorInterno">
        @forelse($productos as $productoUsuario)
            <div class="elemento"><h3> {{ $productoUsuario->producto->titulo }}</h3>
                <div><strong>Tipo: {{ ucfirst($productoUsuario->producto->tipo->nombre) }} </strong></div>
